	...
	public BufferedImage draw(JViewport vp) {
		...
		if (_robotX != -1f && _robotY != -1f) {
			...
			if (collision.getX() != -1) {
				...
				System.out.println("Terrain covered="+computeCoverage()+"%");
				_robotX = _robotY = -1;
			}
			else {
				...
				try {
					...
					while((_robotX != 1 && _robotY != -1) && (System.currentTimeMillis()-startTime)<10000) {
						...
						if (_automaticItem.isSelected()) {
							_angle = w.getValue(left, front, right); 
							// Previous code
					...
					_interface.fitness = computeCoverage();
					latch.countDown();
					//System.exit(0);
...
//		}
		...
		_interface._frame.dispose();
		
	}
	
	public double moveNoGUI() {
		final double[] fitness = new double[1];
		Thread worker = new Thread() {
			public synchronized void run() {
				try {
	...
	public void updateCoverage() {
		int i = (int) (_robotX/(_width/10));
		int j = (int) (_robotY/(_height/10));
		int k = i+10*j;
		if(k < 100) {
			this.coverage[k] = true;
		}
		//System.out.println("_robotX="+_robotX+"; i="+i+"; j="+j+"; k="+k);
	}
